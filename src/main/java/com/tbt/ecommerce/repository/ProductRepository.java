...
public interface ProductRepository extends JpaRepository<Product, Long> {
    @Query("SELECT p FROM Product p WHERE (p.category.name=:category OR :category='' OR :category IS NULL) AND ((:minPrice IS NULL AND :maxPrice IS NULL) OR (p.discountedPrice BETWEEN :minPrice AND :maxPrice)) AND (:minDiscount IS NULL OR p.discountPercent>=:minDiscount) ORDER BY CASE WHEN :sort='price_low' THEN p.discountedPrice END ASC, CASE WHEN :sort='price_high' THEN p.discountedPrice END DESC")
